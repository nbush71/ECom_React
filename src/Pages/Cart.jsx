      ...
      <main className="books__main">
        <div className="books__container">
              ...
              {cart.map((book) => (
                <div className="cart__item" key={`${book.id}-${book.title}`}>
                  <div className="cart__book">
                    <img className="cart__book--img" src={book.url} alt={book.title} />
                    <div className="cart__book--info">
                      <span className="cart__book--title">{book.title}</span>
                      <span className="cart__book--price">${((book.salePrice || book.originalPrice) * (book.quantity || 1)).toFixed(2)}</span>
          ...
          {cart.length > 0 && (<div className="total">
            ...
            <button className="btn btn__checkout no-cursor" onClick={() => alert('Have not gotten around to doing this.')}>Proceed to Checkout</button>
          </div>)}
        </div>
      </main>
    </div>
